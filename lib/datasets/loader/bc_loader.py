...
class BCLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sub_label = []
        if self.configer.exists('boundary') and self.configer.get('boundary', 'use_bc'):
            for i, resolution in enumerate(self.configer.get('boundary', 'resolution')):  # "use_be": [2,4,8]   [1,2,4,8]   [1,4,8] ...
                if resolution==1 :
                    sub_label.append(labelmap)
                else:
                    kernel = stride = [resolution, resolution]
                    sub_label.append(self.get_downsample_label(labelmap, kernel, stride, 1/resolution))
        else:
            
            raise argparse.ArgumentTypeError('Wrong Boundary setting...')
        
        return img, labelmap, sub_label
            


    def get_downsample_label(self, input, kernel_size, stride, lower_bound):
        label = torch.nn.functional.avg_pool2d(input, kernel_size, stride)
        label = label>lower_bound
        return label.float()
