...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', default= 0, nargs='+', type=int,
                        dest='gpu', help='The gpu list used.')  # default= [0]
    parser.add_argument('--wandb_state', default= True, type=str2bool,
                        dest='wandb_state', help='The state of wandb.')
    ...
    parser.add_argument('--checkpoints_dir', default=None, type=str,
                        dest='checkpoints:checkpoints_dir', help='The specific dir of model save path.')
    ...
    parser.add_argument('--resume_wandbid', default=None, type=str,
                        dest='network:resume_wandbid', help='resume load wandbid')
    ...
    if args_parser.seed is not None:   # seed = 304
    ...
    # wandb
    if configer.get('wandb_state'):
        os.environ['WANDB_MODE'] = 'online'
    else:
        os.environ['WANDB_MODE'] = 'offline'

    if configer.get('network', 'resume') is None:
        wandb.init(project="sarbud",
                name=configer.get('checkpoints', 'checkpoints_name'),
                resume=False)    
    else:
        wandb.init(project="sarbud",
                   name=configer.get('checkpoints', 'checkpoints_name'),
                   id=configer.get('network', 'resume_wandbid'),
                   resume=True)
        Log.info('Resume Wandb id:{} \n'.format(configer.get('network', 'resume_wandbid')))
    ...
    #          
    # model = None
    if configer.get('method') == 'segmentor':    
        
        if configer.get('phase') == 'train':
            from segmentor.trainer_sar import Trainer
        ...
        elif configer.get('phase') == 'test_large':
            from segmentor.tester_large import Tester_large 
            model = Tester_large(configer)                
    ...
    if configer.get('phase') == 'train':
        ...
        # wandb.finsh()
    # large test
    elif configer.get('phase').startswith('test_large') and configer.get('network', 'resume') is not None:
        model.test_large() 
    # test
